...
# Normalise using training set only

mean = X_train.mean(axis=(0, 1, 2), keepdims=True)
std = X_train.std(axis=(0, 1, 2), keepdims=True) + 1e-6

# Dataset / DataLoader (same augmentation as 04)
...
# Visualise CBAM's learned spatial attention maps
...
if len(attention_selection) > 0:
    ...
    for ax, (_, row) in zip(axes, attention_selection.iterrows()):
        ...
        input_tensor = ((input_tensor - torch.tensor(mean.reshape(3, 1, 1))) /torch.tensor(std.reshape(3, 1, 1))).unsqueeze(0).to(DEVICE)

        with torch.no_grad():
            _, spatial_map = model(input_tensor, return_attention=True)
        attention_map = spatial_map[0, 0].cpu().numpy()
        attention_resized = np.asarray(Image.fromarray((attention_map * 255).astype("uint8")).resize((IMAGE_SIZE, IMAGE_SIZE), resample=Image.BILINEAR), dtype="float32") / 255.0
