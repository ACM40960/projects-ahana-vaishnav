# Exports trained model weights and preprocessing config for all three
# deployable models into the deployment/artifacts/ folder.
# Run this after training scripts 04, 07, and 10 have completed.
#
# ViT (script 11) is excluded — its weights are 330MB and can't be
# shipped in the deployment folder. The app shows it in the comparison
# table but marks it as unavailable for live prediction.
...
trainable = metadata[
    (metadata["usable"] == 1) & (metadata["class_name"].isin(["lens", "presumed_non_lens"]))
].copy()
...
for cfg in MODELS:
    ...
    if cfg["key"] == "mobilenet":
        # Copy weights directly — the MobileNet class wraps torchvision's
        # MobileNetV3 and the key names depend on exact torchvision version,
        # so re-saving through a class instance risks a key mismatch.
        # The app loads with strict=False to handle minor version differences.
