...
Grad-CAM explainability for the real-data CNN (04_train_real_data_cnn.py),
plus a direct check of the brightness-shortcut risk flagged during data
cleaning: does the model's predicted probability correlate with the
center/edge brightness ratio (i.e. is it partly just responding to
overall brightness rather than arc/ring structure)?

Run this AFTER 04_train_real_data_cnn.py has produced
models/real_data_cnn/best_real_data_cnn.pt

Produces:
  results/real_data_cnn/gradcam_grid.png            (heatmaps: correct + incorrect predictions)
  results/real_data_cnn/gradcam_probable_grid.png    (heatmaps on the held-out probable tier)
  results/real_data_cnn/brightness_vs_prediction.png (shortcut-learning check)
  results/real_data_cnn/false_positive_negative_gallery.png
"""
...
# --------------------------------------------------
# 1. Paths — same as 04_train_real_data_cnn.py
# --------------------------------------------------
...
# --------------------------------------------------
# 2. Rebuild the exact same train/val/test split
#    (same seed, same filtering logic as 04_train_real_data_cnn.py)
# --------------------------------------------------
...
# --------------------------------------------------
# 3. Model definition (must match 04_train_real_data_cnn.py exactly)
# --------------------------------------------------
...
# --------------------------------------------------
# 4. Grad-CAM implementation (hooks on the last conv layer)
# --------------------------------------------------

TARGET_LAYER = model.features[6]  # final Conv2d(32, 64, ...), pre-ReLU output at index 6 is the conv; ReLU is index 7
...
def grad_cam(image_array: np.ndarray):
    """Returns (probability, cam_heatmap as HxW array in [0,1])."""
    ...
    # Resize CAM (small feature-map resolution) up to image resolution.
...
# --------------------------------------------------
# 5. Get test-set predictions (needed to pick correct/incorrect examples)
# --------------------------------------------------
...
# --------------------------------------------------
# 6. Grad-CAM grid: correct + incorrect predictions
# --------------------------------------------------
...
# --------------------------------------------------
# 7. Grad-CAM on the held-out 'probable' tier
# --------------------------------------------------

if len(held_out_probable) > 0:
    probable_sample = held_out_probable.sample(
        n=min(8, len(held_out_probable)), random_state=RANDOM_SEED
...
# --------------------------------------------------
# 8. False positive / false negative gallery (plain images, no CAM —
#    useful as a simple "what does the model get wrong" figure)
# --------------------------------------------------

false_positives = test_rows[
    (test_rows["true_label"] == 0) & (test_rows["predicted_label"] == 1)
]
false_negatives = test_rows[
    (test_rows["true_label"] == 1) & (test_rows["predicted_label"] == 0)
]

fp_fn_sample = pd.concat([
    false_positives.head(8),
    false_negatives.head(8),
]).reset_index(drop=True)
...
# --------------------------------------------------
# 9. Brightness-shortcut check: predicted probability vs
#    center/edge brightness ratio on the test set.
# --------------------------------------------------
...
print(f"\nPearson correlation (predicted probability vs. brightness ratio): r={correlation:.3f}, p={p_value:.4g}")
...
print(f"\nSaved brightness-vs-prediction plot to {RESULTS_DIR / 'brightness_vs_prediction.png'}")