...
Vision Transformer comparison model: a frozen pretrained ViT-B/16
...
This is architecturally very different from every CNN variant in this
project (including the CBAM-attention one): instead of convolutions
sliding local filters over the image, a ViT splits the image into
fixed-size patches and uses self-attention to learn relationships
between ALL patches simultaneously, with no built-in assumption that
nearby pixels matter more than distant ones. This gives you a genuine
"different architecture family" comparison point, not just another
CNN variant.

Same splits (re-derived on the same underlying data with the same
seed), class weighting, thresholding, and output format as
04_train_real_data_cnn.py for a fair comparison.

Note: ViT-B/16 expects 224x224 input (not the 128x128 used elsewhere
in this project), so images are loaded fresh at that resolution here
rather than reusing the 128x128 arrays from other scripts.
...
# --------------------------------------------------
# 3. Same train/val/test split logic as 04_train_real_data_cnn.py
# (split on indices/labels only here — images are loaded lazily
# per-batch at 224x224 by the Dataset below, since loading the full
# set eagerly at this resolution would use much more memory than the
# 128x128 arrays used elsewhere).
# --------------------------------------------------

train_idx, temp_idx = train_test_split(
    np.arange(len(trainable)), test_size=0.30, random_state=RANDOM_SEED, stratify=y_all
)
val_idx, test_idx = train_test_split(
    temp_idx, test_size=0.50, random_state=RANDOM_SEED, stratify=y_all[temp_idx]
)
...
# --------------------------------------------------
# 4. Dataset — loads images lazily at 224x224, ImageNet-normalised
# (required for the pretrained ViT backbone).
# --------------------------------------------------
...
# --------------------------------------------------
# 5. Model — frozen ViT-B/16 backbone + small trainable head
# --------------------------------------------------
...
class ViTTransferModel(nn.Module):
    def __init__(self):
        ...
        self.backbone = backbone  # keep the whole ViT (needed to reuse its
                                   # patch embedding + encoder for the CLS token)
    ...
    def forward(self, x):
        # Reproduce torchvision's internal ViT forward up to the CLS
        # token output, without the original 1000-class ImageNet head.
...
# --------------------------------------------------
# 10. Attention-map visualisation (CLS token's attention over
# patches, from the last encoder block) — the ViT analogue of
# Grad-CAM/CBAM's spatial maps.
#
# torchvision's ViT disables attention-weight computation by
# default for speed (need_weights=False). We temporarily patch the
# last block's self-attention to force it on, purely for this
# visualisation step.
# --------------------------------------------------

def get_attention_map(model: ViTTransferModel, image_tensor: torch.Tensor):
    """Returns (probability, attention_grid) or (probability, None) if
    attention capture fails for any reason — visualisation is a bonus,
    not required for the model to work."""
    ...
    try:
        ...
        # weights shape: (batch, seq_len, seq_len). CLS token is index 0;
        # its attention to all patch tokens (excluding itself) is what
        # we visualise.
...
if len(attention_selection) > 0:
    ...
    if any_attention_captured:
        print(f"\nSaved ViT attention grid to {RESULTS_DIR / 'vit_attention_grid.png'}")
    else:
        print(
            "\nAttention-weight capture did not succeed for this torch/torchvision "
            "version — saved plain images without attention overlay instead. "
            "The classification results above are unaffected; this only impacts "
            "the bonus attention visualisation."
        )